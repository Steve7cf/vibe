/* ══════════════════════════════════════════════════════════════════
   Player — queue · crossfade · skip tracking · smart shuffle
   ══════════════════════════════════════════════════════════════════ */
const Player = {
  currentTrack:     null,
  currentIndex:     -1,
  ...
  _tickLast:        0,
  _trackStartTime:  0,
  _smartHistory:    new Set(),

  // Cached DOM references — filled once on first _onTick, never queried again
  _dom: null,
  _cacheDom() {
    if (this._dom) return;
    this._dom = {
      ...
      npSeekFill:document.getElementById('np-seek-fill'),
  ...
  play()   {
    if (!this.currentTrack && this.queue.length) { this._play(0); return; }
    AudioEngine.play().then(() => { this.isPlaying = true; this._updateUI(); });
  },
  ...
  async next(userInitiated = false) {
    ...
    n >= this.queue.length
      ? (this.repeat === 'all' ? this._play(0) : this.stop())
      : this._play(n);
  },
  ...
  prev() {
    ...
    p < 0
      ? (this.repeat === 'all' ? this._play(this.queue.length - 1) : AudioEngine.seekTo(0))
      : this._play(p);
  },
  ...
  addToQueue(track) {
    ...
    UI.renderQueue(); Utils.toast(`Queued "${track.title || '?'}"`);
  },
  playNext(track) {
    this.queue.splice(this.currentIndex + 1, 0, track);
    this.originalQueue.push(track);
    UI.renderQueue(); Utils.toast(`"${track.title || '?'}" plays next`);
  },
  ...
  toggleShuffle() {
    ...
    if (this.shuffle) {
      this._doShuffle(this.currentIndex);
    } else {
  ...
  _onError() {
    if (window.vibeAPI?.log) window.vibeAPI.log('WRN', '[player] error on', this.currentTrack?.path || '?');
  ...
  // Throttled tick — 250ms cap, cached DOM refs, skipped while seek-dragging
  _onTick() {
    ...
    if (d.seekFill) d.seekFill.style.width = w;
    if (d.timeCur)  d.timeCur.textContent  = Utils.formatTime(cur);
    if (d.timeTot && dur) d.timeTot.textContent = Utils.formatTime(dur);

    if (!d.nowPlaying.classList.contains('hidden')) {
      if (d.npSeekFill) d.npSeekFill.style.width = w;
      if (d.npCur)      d.npCur.textContent       = Utils.formatTime(cur);
    }
    ...
    const cooldownOk = (now - this._crossfadeAt) > (xfadeSec + 5) * 1000;
  ...
  _updateRepeatBtn() {
    ...
    if (this.repeat === 'one') {
      btn.appendChild(Object.assign(document.createElement('span'), { className: 'r1-badge', textContent: '1' }));
    }
  },
  ...
  _updateUI() {
    ...
    ['btn-like', 'np-like'].forEach(id => {
      const el = document.getElementById(id); if (el) el.classList.toggle('liked', liked);
    });
